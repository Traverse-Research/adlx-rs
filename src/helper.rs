...
use crate::bindings::{self, adlx_result_to_error, ADLX_VER_MAJOR, ADLX_VER_MINOR, ADLX_VER_RELEASE, ADLX_VER_BUILD_NUM};
...
unsafe fn load_adlx_dll() -> anyhow::Result<AdlxFunctions>  {
    let dll_name = CStr::from_bytes_with_nul(bindings::ADLX_DLL_NAME).unwrap().to_str().unwrap();
...
impl Drop for AdlxHelper {
    fn drop(&mut self) {
        unsafe {
            adlx_result_to_error((self.functions.terminate_fn.unwrap())()).expect("Failed to terminate ADLX");
        }
    }
}
...
pub fn init_helper() -> anyhow::Result<AdlxHelper> {
    ...
    let system = unsafe {
        ...
        let init_full_version = ((ADLX_VER_MAJOR as u64) << 48) | ((ADLX_VER_MINOR as u64) << 32) | ((ADLX_VER_RELEASE as u64) << 16)  | (ADLX_VER_BUILD_NUM as u64);
